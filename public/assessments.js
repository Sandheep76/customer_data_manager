...
async function loadAssessTypeClientDropdown() {
  try {
    ...
    select.innerHTML = '<option value="all">-- All Clients --</option>' + activeClients.map(client => `<option value="${client.id}">${escapeHtml(client.client_name)}</option>`).join("");
    const modalSelect = document.getElementById("assessTypeClientSelectModal");
    modalSelect.innerHTML = '<option value="">-- Select a Client --</option>' + activeClients.map(client => `<option value="${client.id}">${escapeHtml(client.client_name)}</option>`).join("");
...
async function loadAssessTypesList(showAll = false) {
  ...
  try {
    ...
    if (clientId === "all") url = `/api/assess-types/all${showAll ? "?showAll=true" : ""}`;
      ...
      if (!clientId) { document.getElementById("assessTypesBody").innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 30px;">Select a client to view assessment types</td></tr>'; return; }
    ...
    console.error(err);
    document.getElementById("assessTypesBody").innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 30px; color: red;">Error loading assessment types.</td></tr>';
  }
}
...
function renderAssessTypesPage() {
  ...
  if (pageData.length === 0) { tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 30px;">No assessment types found.</td></tr>'; return; }
  tbody.innerHTML = pageData.map(at => {
    ...
    return `<tr ondblclick="editAssessType(${at.id})" style="cursor: pointer;">
...
function renderAssessPagination() {
  ...
  if (totalAssessPages <= 1) { container.classList.add("hidden"); infoContainer.classList.add("hidden"); return; }
  ...
  for (let i = start; i <= end; i++) html += `<button onclick="changeAssessPage(${i})" class="${i === currentAssessPage ? "active" : ""}">${i}</button>`;
...
function showAddAssessTypeModal() {
  ...
  if (!clientId || clientId === "all") { showToast("Please select a specific client first", "error"); return; }
...
async function editAssessType(id) {
  try {
    ...
    document.getElementById("assessTypeModal").style.display = "block";
  } catch (err) { showToast("Error loading assessment type: " + err.message, "error"); }
}
...
async function saveAssessType() {
  ...
  if (!assessTypeData.assess_type) { showToast("Assessment Type Code is required", "error"); return; }
  try {
    let response;
    if (id) response = await fetch(`/api/assess-types/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(assessTypeData) });
    else response = await fetch("/api/assess-types", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(assessTypeData) });
    if (response.ok) {
  ...
  } catch (err) { showToast("Error saving assessment type: " + err.message, "error"); }
}
...
function closeAssessTypeModal() { document.getElementById("assessTypeModal").style.display = "none"; }

async function bulkDeleteAssessTypes() {
  const checkboxes = document.querySelectorAll(".assess-select-cb:checked");
  if (checkboxes.length === 0) { showToast("Please select at least one assessment type.", "info"); return; }
...
async function exportAssessTypes(format) {
  ...
  if (select.value === "all") window.open(`/api/assess-types/all/export?format=${format}&showAll=${showAllAssessTypes ? "true" : "false"}`, "_blank");
    ...
    if (!currentAssessTypeClientId || currentAssessTypeClientId === "all") { showToast("Please select a client first", "error"); return; }
    window.open(`/api/assess-types/export/client/${currentAssessTypeClientId}?format=${format}&showAll=${showAllAssessTypes ? "true" : "false"}`, "_blank");
  }
}
