...
async function loadClientDropdown() {
  try {
    ...
    select.innerHTML = '<option value="">-- Select a Client --</option>' + clients.map(c => `<option value="${c.id}">${escapeHtml(c.client_name)}</option>`).join("");
...
async function loadAssessTypesForProject() {
  ...
  if (!clientId) { select.innerHTML = '<option value="">-- Assess Type --</option>'; return; }
  try {
    ...
    else select.innerHTML = '<option value="">-- Assess Type --</option>' + assessTypes.map(at => `<option value="${at.assess_type}">${escapeHtml(at.assess_type)}</option>`).join("");
  } catch (err) { console.error(err); }
}
...
async function loadProjectMappingTemplates() {
  ...
  if (!clientId) { templateSelect.innerHTML = '<option value="">-- Mapping Template --</option>'; return; }
  try {
    ...
    else templateSelect.innerHTML = '<option value="">-- Mapping Template --</option>' + mappings.map(m => `<option value="${escapeHtml(m.mapping_name)}">${escapeHtml(m.mapping_name)}</option>`).join("");
  } catch (err) { console.error(err); }
}
...
async function uploadExcel() {
  ...
  if (!clientId) { showToast("Please select a client first", "error"); fileInput.value = ""; return; }
  const mappingName = document.getElementById("mappingTemplateSelect").value;
  if (!mappingName) { showToast("Please select a mapping template first", "error"); fileInput.value = ""; return; }
  const formData = new FormData();
  formData.append("excel", file);
  const uploadStatus = document.getElementById("uploadStatus");
  uploadStatus.innerHTML = "<p>📤 Uploading and importing...</p>";
  try {
    const response = await fetch(`/api/upload/${clientId}?assessType=auto&mappingName=${encodeURIComponent(mappingName)}`, { method: "POST", body: formData });
    const result = await response.json();
    if (response.ok) {
      uploadStatus.innerHTML = `<p class="status-success">✅ ${escapeHtml(result.message)}</p>`;
      if (result.summary) uploadStatus.innerHTML += `<p class="status-success">Projects: ${result.summary.projects_created}, Contacts: ${result.summary.contacts_created}, Stakeholders: ${result.summary.stakeholders_created}</p>`;
      clearStatusMessage("uploadStatus");
      loadProjects();
    } else uploadStatus.innerHTML = `<p class="status-error">❌ Error: ${escapeHtml(result.error)}</p>`;
  } catch (err) { uploadStatus.innerHTML = `<p class="status-error">❌ Upload failed: ${escapeHtml(err.message)}</p>`; }
  fileInput.value = "";
}
...
async function loadProjects() {
  ...
  if (!currentClientId) { showToast("Please select a client first", "error"); listContainer.classList.add("hidden"); return; }
  try {
    const response = await fetch(`/api/clients/${currentClientId}/projects`);
    const projects = await response.json();
    allProjects = projects;
    if (allProjects.length === 0) { listContainer.classList.add("hidden"); showToast("No projects found for this client.", "info"); return; }
    listContainer.classList.remove("hidden");
    currentPage = 1;
    totalPages = Math.ceil(allProjects.length / itemsPerPage);
    renderProjectsPage();
    renderPagination();
  } catch (err) { showToast("Error loading projects: " + err.message, "error"); }
}
...
function renderProjectsPage() {
  ...
  if (pageProjects.length === 0) { tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 30px; color: #666;">No projects found.</td></tr>'; return; }
  tbody.innerHTML = pageProjects.map(p => `<tr ondblclick="editProject(${p.id})" style="cursor: pointer;">
...
function renderPagination() {
  ...
  if (totalPages <= 1) { container.classList.add("hidden"); infoContainer.classList.add("hidden"); return; }
  container.classList.remove("hidden"); infoContainer.classList.remove("hidden");
...
function showAddProjectModal() {
  ...
  if (!currentClientId) { showToast("Please select a client first", "error"); return; }
...
async function loadProjectAssessTypes() {
  ...
  try {
    ...
    else select.innerHTML = '<option value="">-- Select Assessment Type --</option>' + assessTypes.map(at => `<option value="${at.assess_type}">${escapeHtml(at.assess_type)} v${at.assess_version}</option>`).join("");
  } catch (err) { console.error(err); }
}
...
async function editProject(projectId) {
  try {
    ...
    setTimeout(() => { document.getElementById("projectAssessType").value = data.project.assess_type || ""; }, 100);
...
async function saveProject() {
  ...
  if (!projectData.customer_name) { showToast("Customer/Company Name is required", "error"); return; }
  if (!projectData.assess_type) { showToast("Assessment Type is required", "error"); return; }
  try {
    let response;
    if (projectId) response = await fetch(`/api/projects/${projectId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(projectData) });
    else response = await fetch("/api/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(projectData) });
    if (response.ok) { showToast("Project saved successfully", "success"); closeProjectModal(); loadProjects(); }
    else { const error = await response.json(); showToast("Error: " + (error.error || "Unknown error"), "error"); }
  } catch (err) { showToast("Error saving project: " + err.message, "error"); }
}
...
async function bulkDeleteProjects() {
  ...
  if (checkboxes.length === 0) { showToast("Please select at least one project.", "info"); return; }
...
async function exportProjects() {
  ...
  try {
    const response = await fetch(`/api/clients/${currentClientId}/projects`);
    const projects = await response.json();
    if (projects.length === 0) { showToast("No projects available to export", "error"); return; }
    const exportData = projects.map(p => ({
      "CI #": p.ci_number, "Project Name": p.project_name, "Project ID": p.project_id,
      "Customer Name": p.customer_name, "Project Type": p.project_type, "Assess Type": p.assess_type,
      "Customer Address": p.address, "Customer City": p.city, "Customer State": p.state,
      "Customer Country": p.cust_country, "Job Address": p.job_address, "Job City": p.job_city,
      "Job State": p.job_state, "Job Country": p.job_country, "Contractor Name": p.contractor_name,
      "Contractor Company": p.contractor_company, "Contractor Phone": p.contractor_phone,
      Stage: p.project_stage, "Upload Date": p.upload_date,
